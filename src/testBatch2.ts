...
async function runBatch2Verification() {
  console.log('====================================================');
  console.log('  STEP 4 — BATCH 2 CHART & TABLE RENDER VERIFICATION');
  console.log('====================================================\n');
  ...
  // -------------------------------------------------------------------------
  // 1. Agriculture Benchmark Chart (LAYOUT_WIDE 1280x720 Canvas)
  // -------------------------------------------------------------------------
  ...
  const agPptxPath = path.join(outputsDir, 'batch2-agriculture.pptx');
  await pptxAg.writeFile({ fileName: agPptxPath });
  const agReport = await validateChartPptx(agPptxPath);

  const agPngPath = path.join(rendersDir, 'batch2-agriculture.png');
  ...
  const waterPptxPath = path.join(outputsDir, 'batch2-mcl.pptx');
  await pptxWater.writeFile({ fileName: waterPptxPath });
  const waterReport = await validateChartPptx(waterPptxPath);

  const waterPngPath = path.join(rendersDir, 'batch2-mcl.png');
  ...
  console.log('====================================================');
  console.log('  BATCH 2 RENDER GENERATION COMPLETE');
  console.log('====================================================');
}

runBatch2Verification().catch((err) => {
  console.error('BATCH 2 RENDER VERIFICATION FAILED:', err);
  process.exit(1);
});
